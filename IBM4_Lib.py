...
class Ser_Iface(object):
    ...
    def SingleChannelSweepA(self, swp_channel, v_strt, v_end, no_steps, v_fixed = 0.0, no_averages = 10):
        ...
        try:
            ...
            if c10:
                ...
                DELAY = 0.05 # timed delay value in units of seconds
    ...
    def talk(self):    
        self.FUNC_NAME = ".talk()" # use this in exception handling messages
        self.ERR_STATEMENT = "Error: " + self.MOD_NAME_STR + self.FUNC_NAME

        try:
            if self.instr_obj.isOpen():

            # --First Attept, didnt work as it would keep returning "Speak"         
            # self.instr_obj.write(b"Speak\r\n")
            # response = self.instr_obj.readline().decode().strip()
            # return response

                # Write to IBM4
                self.instr_obj.write(b"Speak\r\n")

                # Reads first line (copy of command, this returns "Speak")
                first = self.instr_obj.readline().decode().strip()

                # Reads response ("Hello Liam")
                response = self.instr_obj.readline().decode().strip()
    ...
    def vdisplay(self, voltage, pp):    
        ...
        try:
            if self.instr_obj.isOpen():
                ...
                # Turns average and peak to peak voltage values into strings to be sent to IBM4
                val = str(voltage)
                ptop = str(pp)

                self.instr_obj.reset_input_buffer()
                # Write to IBM4
                self.instr_obj.write(f"Einit:{val}:{ptop}\r\n".encode())

                # Gives time for a reponse if receiving text from IBM4
                time.sleep(2)

                # Was used for troubleshooting
                # available = self.instr_obj.in_waiting
                # if available:
                #     raw = self.instr_obj.read(available)
                #     # print("Raw bytes:", raw)
    ...
    # Will make tbis more general in future (able to read from any of the A input pins)
    def read_and_display_A2(self):    
        self.FUNC_NAME = "read_and_display_A2" # use this in exception handling messages
        self.ERR_STATEMENT = "Error: " + self.MOD_NAME_STR + self.FUNC_NAME

        try:
            if self.instr_obj.isOpen():
                # Write to IBM4
                self.instr_obj.write(b"MultiM\r\n")
            else:
                # Do nothing, no link to IBM4 established
                pass
        except Exception as e:
    ...
    def DC_level(self, val, output):    
            self.FUNC_NAME = ".DC_level()" # use this in exception handling messages
            self.ERR_STATEMENT = "Error: " + self.MOD_NAME_STR + self.FUNC_NAME
            ...
            try:
                ...
                # return response
                    ...
                    # Write to IBM4
                    self.instr_obj.write(f"DC:{output}:{val}\r\n".encode())
